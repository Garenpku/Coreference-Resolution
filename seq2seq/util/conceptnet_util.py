...
class ConceptNet:
    ...
    def find_adjacent(self, concept, vocab, k, stopword=None):
        if concept not in self.cpt_dict.keys():
            return []
        raw = list(set(list(self.cpt_dict[concept].keys())))
        res = [word for word in raw if word in vocab]
        if stopword:
            res = [word for word in res if word not in stopword]
        return res[:k]
    ...
    def expand_list(self, concept_list, vocab=False, k=20, stopword=None):
        res = []
        index = []
        for cpt in concept_list:
            res.append([cpt] + self.find_adjacent(cpt, vocab, k, stopword)[:-1])
